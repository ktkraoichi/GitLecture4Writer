...
- Initialize this repository with a README\
  「クローンして使うならここにチェック入れてね」と書いてあります。チェックを入れてください。
...
※何時書いた、というのはちょっと工夫すると自動的にコミットメッセージへ追加できますが、設定ファイルをいじる必要があるので今回は見送ります。
...
中身がテキストではないファイル（バイナリファイル）を扱うのは苦手です。具体的には拡張子が .docx とか .rtf とか .jtd とか .pages とかになっているファイルです。

特に理由がないなら、まずは原稿ファイルの形式をプレーンテキストファイル（.txt）にするのがいいでしょう。
...
日本語の小説でよく使われる、ルビ、圏点、縦中横といった書式を扱いたいなら、青空文庫記法か、マークダウン記法を拡張した「でんでんマークダウン」をオススメします。\
