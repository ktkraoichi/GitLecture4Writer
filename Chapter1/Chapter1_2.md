...
前回の記事で `clone` した「GitLecture4Writer」のローカルリポジトリを更新してみましょう。\
今回のゴールからはちょっと逸れますが、いずれ目指す状態はこんな感じです。
...
ただし、いったんパブリックリポジトリ（公開設定）にするとプライベートリポジトリには戻せません。注意してくださいね。
...
**Initial commit** というコミットがあることが確認できましたか？\
README の内容はリモートリポジトリにある内容と一致していますか？
...
ちなみに。\
お気づきかもしれませんが、 CLI を使った Git の操作は、 GUIに比べるとステップ数がめちゃくちゃ少なくて済みます。
